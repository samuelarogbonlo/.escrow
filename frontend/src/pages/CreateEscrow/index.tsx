...
const CreateEscrow = () => {
  ...
  const { createEscrow, selectedAccount, isExtensionReady, connectExtension } = useWallet();
  ...
  useEffect(() => {
    if (currentStep === 3) {
      if (!isExtensionReady) {
        setWalletError("Wallet extension not connected. Please connect to create an escrow.");
      } else if (!selectedAccount) {
        setWalletError("No account selected. Please select an account to create an escrow.");
      } else {
        setWalletError(null);
      }
    }
  ...
  const handleConnectWallet = async () => {
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Unable to connect to wallet',
        status: 'error',
        duration: 5000,
        isClosable: true,
      });
  ...
  const validateStep = (): boolean => {
    ...
    if (currentStep === 0) {
      ...
      } else if (Math.abs(totalMilestoneAmount - Number(formData.totalAmount)) > 0.01) {
      ...
      formData.milestones.forEach((milestone, index) => {
        ...
        if (!milestone.amount || !milestone.amount.trim()) {
          ...
          errors[`milestone_${index}_amount`] = 'Amount must be a positive number';
        }
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedAccount) {
      setWalletError("Wallet not connected. Please connect your wallet before creating an escrow.");
      toast({
    ...
    try {
      ...
      const worker = formData.counterpartyType === 'worker' ? counterpartyAddress : userAddress;
      const client = formData.counterpartyType === 'client' ? counterpartyAddress : userAddress;
      
      // Format milestones for contract
      const milestones = formData.milestones.map((m) => ({
        description: m.description,
        amount: m.amount,
        deadline: m.deadline ? m.deadline.getTime() : Date.now() + 30 * 24 * 60 * 60 * 1000, // 30 days from now as fallback
      }));
  ...
  return (
    <Box>
      <Heading size="lg" mb={6}>Create Escrow</Heading>
