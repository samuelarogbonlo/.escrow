...
interface WalletContextType {
  ...
  // Escrow contract
  createEscrow: (worker: string, client: string, totalAmount: string, milestones: any[]) => Promise<any>;
  ...
  disputeMilestone: (escrowId: string, milestoneId: string, reason: string) => Promise<any>;
}
...
export const WalletProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [mockAccount, setMockAccount] = useState<InjectedAccountWithMeta | null>(null);
  ...
  useEffect(() => {
    ...
    if (api.api && (extension.isReady || mockAccount)) {
      dispatch(setWallet({
        api: api.api,
        extension: mockAccount?.meta.source || extension.accounts[0]?.meta.source || null,
      }));
    }
  }, [api.api, extension.isReady, extension.accounts, mockAccount, dispatch]);
